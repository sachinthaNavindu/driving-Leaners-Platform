...
async function loadTableAndOtherStats() {
  try {
    const response = await fetchWithAuth("http://localhost:8080/admin/getVehicles");
    if (!response) return; 
    ...
    document
      ...
      .querySelector(".stat-card-value").textContent = data.availableVehicleCount;
    document
      .querySelectorAll(".stat-card")[1]
      .querySelector(".stat-card-value").textContent = data.unavailableVehicleCount;
    ...
    data.vehicles.forEach((vehicle) => {
      ...
      viewBtn.addEventListener("click", () => loadVehicleToForm(vehicle, false));

      const editBtn = document.createElement("button");
      editBtn.innerHTML = '<i class="fas fa-edit"></i>';
      editBtn.classList.add("action-btn", "btn-edit");
      editBtn.title = "Edit Vehicle";
      editBtn.addEventListener("click", () => loadVehicleToForm(vehicle, true));

      const deleteBtn = document.createElement("button");
      deleteBtn.innerHTML = '<i class="fas fa-trash"></i>';
      deleteBtn.classList.add("action-btn", "btn-delete");
      deleteBtn.title = "Delete Vehicle";
      deleteBtn.addEventListener("click", () => showDeleteConfirmation(vehicle.vehicleNumber));
    ...
    showMessage("Failed to load vehicle data. Please try again.", "error");
  }
}
...
function loadVehicleToForm(vehicle, isEditable = false) {
  ...
  if (isEditable) {
    ...
    submitBtn.innerHTML = '<i class="fas fa-sync-alt"></i> Update Vehicle';
    submitBtn.style.background = "linear-gradient(135deg, #1a2a6c, #b21f1f)";
    submitBtn.onclick = (e) => {
      e.preventDefault();
  ...
  try {
    const response = await fetchWithAuth(
      `http://localhost:8080/admin/updateVehicle/${vehicleNumber}`,
      {
        ...
        body: JSON.stringify({ vehicleNumber, availability, vehicleType }),
      }
    );
    if (!response) return;
...
function resetFormToAddMode() {
  ...
  cancelBtn.onclick = () => document.getElementById("addVehicleForm").reset();
}
...
document.getElementById("addVehicleForm").addEventListener("submit", async (e) => {
  ...
  try {
    const response = await fetchWithAuth("http://localhost:8080/admin/vehicleSave", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ vehicleNumber, availability, vehicleType }),
    });
