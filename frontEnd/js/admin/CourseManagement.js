...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const courseToDeleteNameEl = document.getElementById("courseToDeleteName");
  ...
  courseForm.addEventListener("submit", async function (e) {
    ...
    try {
      const response = await fetchWithAuth("http://localhost:8080/admin/saveCourse", {
        method: "POST",
  ...
  async function deleteCourse() {
    ...
    try {
      const response = await fetchWithAuth(
        `http://localhost:8080/admin/courseDelete/${encodeURIComponent(courseToDeleteName)}`,
        { method: "DELETE" }
      );
  ...
  window.viewCourse = function (courseName) {
    ...
    document.getElementById("add-course").scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  function enterViewMode(course) {
    ...
    submitButton.style.background = "linear-gradient(135deg, #6c757d, #495057)";
  ...
  function exitViewMode() {
    ...
    const formHeader = document.querySelector(".form-header h2");
    formHeader.innerHTML = '<i class="fas fa-plus-circle"></i> Add New Course';

    const submitButton = document.querySelector(".btn-register");
    submitButton.innerHTML = '<i class="fas fa-save"></i> Save Course';
    submitButton.style.background = "linear-gradient(135deg, #1a2a6c, #b21f1f)";
  ...
  function enterEditMode(course) {
    ...
    submitButton.style.background = "linear-gradient(135deg, #ff8a4c, #b21f1f)";
    document.querySelector(".course-id-display").style.display = "block";
  }
  ...
  async function updateCourse() {
    ...
    try {
      const response = await fetchWithAuth(
        `http://localhost:8080/admin/courseUpdate/${encodeURIComponent(currentEditCourseName)}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(courseData),
